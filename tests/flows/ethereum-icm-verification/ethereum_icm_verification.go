...
func EthereumICMVerification(
	...
	log.Info("Adding new validator")
	avalancheNetwork.AddSubnetValidators(
		avalancheNetwork.GetExtraNodes(1),
		l1Info,
		false,
	)

	log.Info("Registering new validator set with validator set registry")
	utils.RegisterAvalancheValidatorSet(
		ctx,
		ethereumFundedKey,
		ethereumNetwork.ChainID,
	...
	// Send and verify an ICM message from the Avalanche L1 network to the Ethereum network after updating
	// the validator set.
	teleporter.SendExampleInterChainMessageAndVerify(
		ctx,
		ethereumNetwork.ChainID,
