...
DEFAULT_ETHEREUM_CONTRACT_LIST="AvalancheValidatorSetRegistry EthWarp"
...
ethereum_remappings=$(cat $ETHEREUM_ICM_PATH/remappings.txt)
...
cd $ETHEREUM_ICM_PATH/contracts
generate_bindings "$ETHEREUM_ICM_PATH" "$ETHEREUM_EVM_VERSION" "$ETHEREUM_SOLIDITY_VERSION" "$ethereum_remappings" "--via-ir" "${contract_names[@]}"
